...
export default function useVCStakingGold() {
    ...
    const VCStakingContractInstance = useMemo(
        () => new web3.eth.Contract(VCStakeABI, VC_STAKE_GOLD_ADDRESS),
        [web3]
    );
    ...
    const onStake = async (refAddress) => {
        ...
        try {
            const allowance = await VCContractInstance.methods
                .allowance(address, VC_STAKE_GOLD_ADDRESS)
                .call();
            if (allowance < depositAmountInWei) {
                await VCContractInstance.methods.approve(VC_STAKE_GOLD_ADDRESS, MAX_LP).send({
                    from: address,
                });
            }
